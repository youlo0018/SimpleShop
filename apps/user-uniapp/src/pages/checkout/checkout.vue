  ...
  <view class="page">
    ...
    <view v-if="settle && ((settle.coupons || []).length || (settle.activities || []).length)" class="card">
      ...
      <view v-for="coupon in settle.coupons || []" :key="coupon.userCouponId" class="promo-row" @tap="toggleCoupon(coupon.userCouponId)">
        <text class="tick" :class="{ on: selectedCouponIds.includes(coupon.userCouponId) }">{{ selectedCouponIds.includes(coupon.userCouponId) ? '✓' : '' }}</text>
    ...
    <view class="bar safe-bottom">
      ...
      <button class="submit" :disabled="submitting" @tap="submit">{{ submitting ? '支付中' : '提交并支付' }}</button>
    </view>
  </view>
</template>
...
onLoad(async options => { items.value = uni.getStorageSync('checkout') || []; requireLogin(); await refreshPreview(true) })
...
.promo-value { color: #ff3b30; font-size: 26rpx; font-weight: 600; }
.item-discount { color: #ff3b30; margin-left: 8rpx; } .amount .yen { font-size: 24rpx; font-weight: 600; margin-right: 2rpx; } .submit { width: 260rpx; background: #0071e3; color: #fff; box-shadow: 0 6rpx 18rpx rgba(0, 113, 227, .3); }</style>
