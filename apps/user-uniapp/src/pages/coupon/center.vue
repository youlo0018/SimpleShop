  ...
  <view class="page">
    ...
    <template v-else>
      ...
      <view v-for="item in items" :key="item.id" class="ticket">
        ...
        <button class="claim" :class="{ off: !item.canClaim }" :style="item.canClaim ? { background: theme.primary } : {}" :disabled="!item.canClaim" @tap="claim(item)">{{ item.canClaim ? '领取' : (Number(item.claimedCount) >= Number(item.perUserLimit) ? '已领取' : '已抢完') }}</button>
      </view>
...
const theme = ref(getTheme()); const platformId = ref(''); const items = ref([])

// 数字统一按字符串下发，类型判断必须先转数字。
...
.ticket { display: flex; align-items: center; background: #fff; border-radius: 26rpx; padding: 30rpx 26rpx; margin-bottom: 22rpx; box-shadow: 0 6rpx 24rpx rgba(0,0,0,.05); }
.left { width: 180rpx; text-align: center; border-right: 1rpx dashed #e5e5ea; }
...
.claim { width: 150rpx; height: 68rpx; line-height: 68rpx; border-radius: 980px; background: #0071e3; color: #fff; font-size: 26rpx; font-weight: 600; padding: 0; box-shadow: 0 6rpx 16rpx rgba(0, 113, 227, .22); }
