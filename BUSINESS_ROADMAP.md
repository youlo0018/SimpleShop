...
| 用户会员 | UserService | 账号资料、收货地址、实名信息、等级成长值、积分、标签、偏好设置 | 骨架 | 拆分“账号身份”与“会员资产”，避免用户表膨胀 |
| 商品 | ProductService | SPU/SKU、类目、品牌、规格、价格、商品详情、上下架、搜索索引同步 | 较完整 | 引入草稿/审核态、价格版本、前台展示快照和搜索读模型 |
| 库存 | InventoryService | 可售库存、锁定、扣减、回补、仓库维度库存、安全库存、超卖控制 | 缺失 | 新建服务；下单预占、支付超时释放、发货扣减、取消回补 |
| 购物车 | CartService | 加购、合并、选中、优惠试算入口、失效商品提示 | 缺失 | C 端高读写场景可用 Redis 为主存储，异步持久化关键变更 |
| 订单交易 | OrderService | 订单创建、订单状态机、拆单、取消、支付超时关闭、订单查询 | 骨架 | 先实现单体订单聚合，再按量拆分履约与售后 |
| 支付 | PaymentService | 收银台、支付单、渠道适配、回调验签、幂等、退款、对账 | 缺失 | 新建服务；支付结果通过事件驱动订单和库存 |
