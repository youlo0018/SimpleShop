...
{
    /// <summary>处理入口：后台用户分页列表：keyword 模糊匹配用户名/手机号/邮箱；total 与取数同条件查询。 平台管理员看全部用户（用户表不按平台切分，角色绑定在权限中心）。</summary>
    public async Task<ApiResponse> Handle(ListUsersQuery request, CancellationToken cancellationToken)
    {
        var (items, total) = await repository.QueryPagedAsync(request.Keyword, request.Page, request.PageSize);
