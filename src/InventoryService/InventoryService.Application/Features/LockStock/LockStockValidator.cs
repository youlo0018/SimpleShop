...
/// <summary>
/// 锁定库存参数校验：BizNo 是流水幂等键，不能为空；空明细会直接"成功"返回，必须拦截。
/// </summary>
public class LockStockValidator : AbstractValidator<LockStockCommand>
{
    public LockStockValidator()
    {
        RuleFor(x => x.BizNo)
        ...
        RuleForEach(x => x.Items).ChildRules(item =>
        {
            ...
            item.RuleFor(i => i.Quantity).InclusiveBetween(1, 10000).WithMessage("锁定数量必须为1-10000");
        });
    }
}
