...
| USER-13 | P1 | 游客 | `GET /gateway/platform-configs/Regions?platformId=`（未配置） | 200；`isCustom=false`，返回内置默认数据（31 省 / 342 市 / 3056 区县） | api |
| USER-14 | P1 | 后台 token | `POST /gateway/platform-configs/SaveRegions` 保存合法三级 JSON → 再读取 | 200；读取 `isCustom=true` 且省市区与保存一致；落库 `platform_app_config.RegionsJson` | api |
| USER-15 | P1 | 后台 token | `SaveRegions` 传空字符串 → 再读取 | 200；`isCustom=false`，读取回落内置默认数据（清除自定义） | api |
| USER-16 | P1 | 后台 token | `SaveRegions` 传非 JSON / 空数组 / 缺少 name | HTTP 400（合法JSON/不能为空数组/缺少名称） | api |
...
| DSN-08 | P1 | 后台装修页 | 「地址地区」Tab：多级下拉浏览 → 新增/删除 → 保存自定义 → 恢复默认 | 级联下拉可逐级浏览；不选新增省/选省新增市/选到市新增区县；删除二次确认；保存后 `isCustom=true`，恢复默认回落 31 省 | ui |
| DSN-09 | P1 | 后台已登录 | 菜单「地区地址」独立页 | 平台下拉/省市区统计（默认 31/342/3056）/级联增删/保存/恢复默认均可用；与装修页 Tab 共用 RegionEditor | ui |
...
| ADM-09 | P1 | 平台租户账号 | 登录后台 | 「平台管理」菜单隐藏；装修/地区地址/商户/营销/用户页的平台下拉与平台列隐藏并锁定本平台；地区统计正常（31/342/3056） | ui |
| ADM-10 | P1 | 商户租户账号 | 登录后台 → 添加商品 | 商品表单商户下拉隐藏（自动本商户）；商品列表/详情商户列隐藏；不请求 /merchants/List | ui |
| ADM-11 | P1 | 后台 token | `GET /gateway/merchants/List?keyword=&page=1&pageSize=10` | 200 返回分页数据（空关键字不得 400） | api |
...
| MP-07 | P1 | 有可用券 | 提交页 | 金额明细（商品金额/优惠合计/实付）、券勾选切换金额浮动、活动只展示最优惠的一个（赠券仅在无折扣活动时兜底） | ui |
...
| MP-12 | P1 | 已登录 | 地址页新增地址：省市区选择 | 省/市/区为三级联动下拉（默认全国数据，首项广东省/深圳市/南山区）；列切换后市/区联动；详细地址仍为手填输入 | ui |
| MP-13 | P1 | 平台已自定义地区 | 清除本地地区缓存后重进地址页 | 下拉使用平台自定义数据（自定义第一项自动回填，找不到的旧值回落第一项） | ui |
| MP-14 | P1 | 已登录 | 我的页点击权益/我的服务图标（购物车/我的订单等） | 跳转对应页面（cart 为 switchTab）；未登录先跳登录；客服类提示筹备中 | ui |
| MP-15 | P1 | 已登录 | 结算页点「请选择收货地址 +」→ 新增地址 | 进入地址页且弹窗含收货人/手机号/省市区下拉/详细地址输入框；详细地址不被底部按钮遮挡且可聚焦输入 | ui |
| MP-16 | P1 | 客户有待收货订单（40，包裹已发货） | 订单列表/详情点「确认收货」→ 确认弹窗 | 提示已确认收货；数据库 `order.OrderStatus=50`、`shipment.Status=50`、`ReceivedAt` 已写；列表状态变已完成 | ui |
...
| 日期 | 脚本 | 结果 |
|------|------|------|
| 2026-09-21 | `ui-regression.js` | 通过 46 / 失败 0（省市区下拉/地区配置改动后回归） |
| 2026-09-21 | `api-regression.sh` | 通过 114 / 失败 0（含商户列表空关键字修复、订单详情 shipments 后回归） |
| 2026-09-21 | 地区编辑器 CDP 实测 | 级联选北京市→新增市 343→删除回 342→新增省保存（isCustom=true，32 省）→恢复默认（31 省） |
| 2026-09-21 | 确认收货 CDP 实测 | 列表 2 个待收货订单出现按钮；确认后订单 50/包裹 50/签收时间已写 |
| 2026-09-21 | 租户视角 CDP 实测 | codexadmin 全量可见；平台管理员隐藏平台下拉/列且统计 31/342/3056；商户账号隐藏商户下拉/列 |
| 2026-09-21 | 地区地址独立页 CDP 实测 | 菜单含「地区地址」；统计 31/342/3056；JSON 128366 字符含北京市 |
| 2026-09-21 | `build:mp-weixin` 产物校验 | `app.json/app.js/app.wxss` + `App2.js/App2.json` 结构完整；`app.wxss` 含 CSS 变量与 safe-bottom；`pages/address` 含 multiSelector 与详细地址 |
| 2026-09-21 | 地区配置 CDP 实测 | 后台 Tab 加载 128KB 默认数据；保存自定义后小程序下拉=北京市/市辖区/朝阳区；恢复默认后回落 31 省 |
